...
# 读取影评文件
with open("我不是药神 Dv.csv", 'r', encoding="utf-8") as fs:
    st = fs.read()
plot_list = []  # 存放该维度下检索到的句子
plot = 0.0  # 存放情感极性数值
for kw in contents:
    pattern = re.compile('[，,。.！？；…（\n]+[^，,。.！？；…（\n]*' + kw
                         + '[^，。.！？；…）\n]*[，！.。？；…）\n]+')
...
print("专业影评库六个维度的数值计算完成！")

# 将六个维度数值转换为字符串后写入文件
with open("Dv.txt", 'w', encoding="utf-8") as fv:
    fv.write(str(plot) + '\n' + str(audio_visual) + '\n' + str(character) + '\n'
             + str(fan) + '\n' + str(commercial) + '\n' + str(conception))
fv.close()
